<script lang="ts">
	...
	let due_date: string = task?.due_date || '';
	let priority: string = task?.priority || '';

	const isValidForm = () => {
		return title.trim() !== '' && description.trim() !== '' && due_date.trim() !== '' && priority.trim() !== '';
	};
...
<form class="form-container" on:submit|preventDefault={updateTask}>
	<h2>Edit Task</h2>
...
<style>
    /* You can reuse the same styles from TaskForm */
    .form-container {
